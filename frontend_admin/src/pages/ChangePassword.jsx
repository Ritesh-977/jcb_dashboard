...
function PasswordInput({ value, onChange, placeholder }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <input type={show ? 'text' : 'password'} required style={{ ...styles.input, paddingRight: '2.5rem' }}
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        <button onClick={() => navigate('/users')} style={styles.back}>← Back to User Management</button>
        <h2 style={styles.title}>Change Password</h2>
        <form onSubmit={handleSubmit}>
          {[['old_password', 'Current Password'], ['new_password', 'New Password']].map(([key, label]) => (
            <div key={key} style={styles.field}>
              <label style={styles.label}>{label}</label>
              <PasswordInput value={form[key]} onChange={(e) => setForm((p) => ({ ...p, [key]: e.target.value }))} placeholder="••••••••" />
            </div>
          ))}
          <div style={styles.field}>
            <label style={styles.label}>Confirm New Password</label>
            <input type="password" required style={styles.input} value={form.confirm}
